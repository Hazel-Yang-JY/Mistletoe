...
# Resolve the checkpoint relative to this module so generation works no matter
# which directory the caller launches the script from.
CHECKPOINT_PATH = os.path.join(os.path.dirname(__file__), "vqvae.bin")
...
def get_rec_image(
    ...
    if image is None:
        raise FileNotFoundError(f"Unable to read input image: {filename}")
    ...
    if not cv2.imwrite(output_path, rec_image):
        raise OSError(f"Unable to write reconstructed image: {output_path}")
